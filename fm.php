...
include(WP_PLUGIN_DIR . "/wp-filemanager/conf/config.inc.php");
include(WP_PLUGIN_DIR . "/wp-filemanager/incl/functions.inc.php");
include(WP_PLUGIN_DIR . "/wp-filemanager/lang/$language.inc.php");
include(WP_PLUGIN_DIR . "/wp-filemanager/incl/header.inc.php");
include(WP_PLUGIN_DIR . "/wp-filemanager/incl/html.header.inc.php");
...
if (isset($_SESSION['session_username']) && $_SESSION['session_username'] == $username && isset($_SESSION['session_password']) && $_SESSION['session_password'] == md5($password) || !$phpfm_auth)
{
    ...
    if (isset($_GET['action']) && is_file(WP_PLUGIN_DIR . "/wp-filemanager/incl/".$_GET['action'].".inc.php") && wp_fileman_is_valid_name($_GET['action']))
	{
        include(WP_PLUGIN_DIR . "/wp-filemanager/incl/".basename($_GET['action']).".inc.php");
	}
    else if (isset($_GET['output']) && is_file(WP_PLUGIN_DIR . "/wp-filemanager/incl/".$_GET['output'].".inc.php") && wp_fileman_is_valid_name($_GET['output']))
    {
        print "<table class='output' width=400 cellpadding=0 cellspacing=0>";
            print "<tr><td align='center'>";
                include(WP_PLUGIN_DIR . "/wp-filemanager/incl/".basename($_GET['output']).".inc.php");
            print "</td></tr>";
        print "</table><br />";
        include(WP_PLUGIN_DIR . "/wp-filemanager/incl/filebrowser.inc.php");
    }
    else
    {
        include(WP_PLUGIN_DIR . "/wp-filemanager/incl/filebrowser.inc.php");
...
{
    include(WP_PLUGIN_DIR . "/wp-filemanager/incl/login.inc.php");
}
//include(WP_PLUGIN_DIR . "/wp-filemanager/incl/footer.inc.php");
?>
