...
include_once(WP_PLUGIN_DIR . "/wp-filemanager/lang/$language.inc.php");
include_once(WP_PLUGIN_DIR . "/wp-filemanager/incl/header.inc.php");
if ($AllowRename && isset($_GET['directory_name']) || $AllowRename && isset($_GET['filename']) || $AllowRename && isset($_POST['directory_name']) || $AllowRename && isset($_POST['filename']))
{
 ...
 {
  check_admin_referer('rename_save_' . $_POST['directory_name']);
//  check_admin_referer('rename_save_dir');
 ...
 {  
  ...
  check_admin_referer('rename_save_' . $filename);
if (!wp_fileman_is_valid_name($new_filename))
 ...
 {
 if (isset($_GET['directory_name']))   check_admin_referer('rename_dir_' . $_GET['directory_name']);
 if (isset($_GET['filename']))   check_admin_referer('rename_file_'. $_GET['filename']);
  ...
  print "<tr>";
    ...
    if (isset($_GET['directory_name'])) print wp_nonce_field('rename_save_' . $_GET['directory_name']) . "<br /><br />";
    else if (isset($_GET['filename'])) print wp_nonce_field('rename_save_' . $_GET['filename']) . "<br /><br />";
    print "</form>";
