...
if ( ! defined( 'ABSPATH' ) )
	die();
//if (isset($_GET['directory_name']))check_admin_referer('remove_dir');
//if (isset($_GET['filename']))check_admin_referer('delete_file');
...
if ($AllowDelete && isset($_GET['directory_name']) || $AllowDelete && isset($_GET['filename']))
{
 ...
 {
  check_admin_referer('remove_dir');
 ...
 {
  check_admin_referer('delete_file');
 ...
 else
 {
   print "<table class='index' width=500 cellpadding=0 cellspacing=0>";
	...
	check_admin_referer('delete_file');
     ...
     if (isset($_GET['directory_name'])) print "<a href='" . wp_nonce_url("$base_url&amp;path=".htmlentities(rawurlencode($wp_fileman_path))."&amp;directory_name=".htmlentities(rawurlencode($directory_name))."&amp;output=delete&amp;delete=true", 'remove_dir') . "'>$StrYes</a>";
     else if (isset($_GET['filename'])) print "<a href='" . wp_nonce_url("$base_url&amp;path=".htmlentities(rawurlencode($wp_fileman_path))."&amp;filename=".htmlentities(rawurlencode($filename))."&amp;output=delete&amp;delete=true",'delete_file'). "'>$StrYes</a>";
     print "&nbsp;$StrOr&nbsp;";
